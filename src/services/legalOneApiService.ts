...
class LegalOneApiService {
    ...
    public async getProcessDetails(processNumber: string): Promise<LegalOneLawsuit> {
        ...
        const apiBaseUrl = `${process.env.LEGAL_ONE_API_BASE_URL}/v1/api/rest/Lawsuits`;
        const headers = { 'Authorization': `Bearer ${token}` };

        // Tentativa 1: com o número original
        try {
            const params = { '$filter': `identifierNumber eq '${cleanProcessNumber}' or otherNumber eq '${cleanProcessNumber}'`, '$expand': 'participants,courtPanel' };
            const res = await axios.get<LegalOneLawsuitApiResponse>(apiBaseUrl, { headers, params });
            if (res.data.value?.length > 0) return res.data.value[0];
        } catch (e) { /* Ignora o erro para tentar a próxima alternativa */ }

        // Tentativa 2: com o número sem máscara
        const unmasked = cleanProcessNumber.replace(/[.\-/]/g, '');
        if (unmasked !== cleanProcessNumber) {
            try {
                const params = { '$filter': `identifierNumber eq '${unmasked}' or otherNumber eq '${unmasked}'`, '$expand': 'participants,courtPanel' };
                const res = await axios.get<LegalOneLawsuitApiResponse>(apiBaseUrl, { headers, params });
                if (res.data.value?.length > 0) return res.data.value[0];
            } catch (e) {
                throw new Error(`Nenhum processo encontrado para o número: ${cleanProcessNumber}`);
            }
        }
    ...
    /**
     * Busca os detalhes de um recurso (apelação) pelo seu número.
     * @param {string} processNumber - O número do recurso.
     * @returns {Promise<LegalOneAppeal>} Os detalhes do recurso.
     */
    public async getAppealDetails(processNumber: string): Promise<LegalOneAppeal> {
        const token = await this.getAccessToken();
        const trimmedNumber = processNumber.trim();
        try {
            const res = await axios.get<LegalOneAppealApiResponse>(`${process.env.LEGAL_ONE_API_BASE_URL}/v1/api/rest/appeals`, {
                headers: { 'Authorization': `Bearer ${token}` },
                params: { '$filter': `identifierNumber eq '${trimmedNumber}' or otherNumber eq '${trimmedNumber}'`, '$expand': 'participants,courtPanel' }
            });
            if (res.data.value?.length > 0) return res.data.value[0];
        } catch (e) { /* Ignora o erro para lançar a exceção padrão */ }
        throw new Error("Recurso não encontrado");
    }

    /**
     * Busca os detalhes de um incidente processual pelo seu número.
     * @param {string} processNumber - O número do incidente.
     * @returns {Promise<LegalOneProceduralIssue>} Os detalhes do incidente.
     */
    public async getProceduralIssueDetails(processNumber: string): Promise<LegalOneProceduralIssue> {
        const token = await this.getAccessToken();
        const trimmedNumber = processNumber.trim();
        try {
            const res = await axios.get<LegalOneProceduralIssueApiResponse>(`${process.env.LEGAL_ONE_API_BASE_URL}/v1/api/rest/ProceduralIssues`, {
                headers: { 'Authorization': `Bearer ${token}` },
                params: { '$filter': `identifierNumber eq '${trimmedNumber}' or otherNumber eq '${trimmedNumber}'`, '$expand': 'participants,courtPanel' }
            });
            if (res.data.value?.length > 0) return res.data.value[0];
