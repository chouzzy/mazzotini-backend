...
export class LegalOneContacts extends LegalOneAuth {
    ...
    public async updateContact(contactId: number, user: User, associateName?: string): Promise<void> {
        ...
        // 1. Remove campos que não podem ir no PATCH raiz
        ...
        try {
            ...
            console.error('Erro update root:', e.message); 
        }

        if (associateName) await this.updateAssociateCustomField(contactId, associateName, isPJ);

        // 2. Busca detalhes para IDs de sub-recursos
        let currentContact: LegalOneContact;
        try { currentContact = await this.getContactDetails(contactId, isPJ); } catch { return; }

        // 3. Atualiza E-mails
        if (newPayload.emails.length > 0) {
            const mainNew = newPayload.emails.find((e: any) => e.isMainEmail) || newPayload.emails[0];
            const mainCurr = currentContact.emails?.find(e => e.isMainEmail) || currentContact.emails?.[0];
            try {
                if (mainCurr) {
                    await axios.patch(`${apiRestUrl}/${endpointType}/${contactId}/emails/${mainCurr.id}`, { ...mainNew, id: mainCurr.id }, { headers });
                } else {
                    await axios.post(`${apiRestUrl}/${endpointType}/${contactId}/emails`, mainNew, { headers });
                }
            } catch (e: any) { console.error('Erro email:', e.message); }
        }

        // 4. Atualiza Telefones
        if (newPayload.phones.length > 0) {
            const mainNew = newPayload.phones.find((p: any) => p.isMainPhone) || newPayload.phones[0];
            const mainCurr = currentContact.phones?.find(p => p.isMainPhone) || currentContact.phones?.[0];
            try {
                if (mainCurr) {
                    await axios.patch(`${apiRestUrl}/${endpointType}/${contactId}/phones/${mainCurr.id}`, { ...mainNew, id: mainCurr.id }, { headers });
                } else {
                    await axios.post(`${apiRestUrl}/${endpointType}/${contactId}/phones`, mainNew, { headers });
                }
            } catch (e: any) { console.error('Erro phone:', e.message); }
        }

        // 5. Atualiza Endereços
        if (newPayload.addresses && newPayload.addresses.length > 0) {
            try {
                const addrRes = await axios.get<LegalOnePagedResponse<LegalOneAddress>>(`${apiRestUrl}/${endpointType}/${contactId}/addresses`, { headers });
                const existingAddrs = addrRes.data.value || [];
                
                for (const newAddr of newPayload.addresses) {
                    let match = newAddr.isMainAddress
                        ? existingAddrs.find(a => a.isMainAddress)
                        : existingAddrs.find(a => !a.isMainAddress && a.type === newAddr.type);

                    if (match) {
                        await axios.patch(`${apiRestUrl}/${endpointType}/${contactId}/addresses/${match.id}`, { ...newAddr, id: match.id }, { headers: { ...headers, 'Content-Type': 'application/json' } });
                    } else {
                        await axios.post(`${apiRestUrl}/${endpointType}/${contactId}/addresses`, newAddr, { headers: { ...headers, 'Content-Type': 'application/json' } });
                    }
                }
            } catch (e: any) { console.error('Erro endereços:', e.message); }
        }
    }
}