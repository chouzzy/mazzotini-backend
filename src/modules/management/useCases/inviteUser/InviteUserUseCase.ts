 ...
 */
class InviteUserUseCase {
    async execute({ email, name, initialRole }: IRequest): Promise<{ ticketUrl: string }> {
        // Passo 1: Cria o utilizador no Auth0 e gera o link de "Crie a sua Senha".
        ...
        console.log(`[INVITE USER] Link de criação de senha para ${email} gerado com sucesso.`);
        console.log(`ticketUrl: ${ticketUrl}`);
        
        // Passo 3: Retorna o link para o Controller.
        return { ticketUrl };
    }
}
