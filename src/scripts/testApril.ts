...
(async()=>{
  ...
  for(const{year:y,month:m}of months){
    let r:number;
    if(y<2003||(y===2003&&m<=1)){r=0.5;p1++;}
    else if(y<2024||(y===2024&&m<=8)){r=1.0;p2++;}
    else{const k=`${y}-${String(m).padStart(2,'0')}`;r=Math.max((sM.get(k)??0)-(iM.get(k)??0),0);p3++;}
    tot+=r;
  }
  const mor=C*tot/100;
  console.log(`Config: P1<=Jan/03 | P2 fev/03-ago/24 | P3 set/24+ | inclui mês base`);
  console.log(`P1=${p1}m  P2=${p2}m  P3=${p3}m  total=${p1+p2+p3}m`);
  console.log(`rate=${tot.toFixed(4)}%`);
  console.log(`Juros nosso: R$ ${mor.toFixed(2)}`);
  console.log(`Juros drcalc: R$ ${T.toFixed(2)}`);
